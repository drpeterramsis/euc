...
// POST /api/checkins/uncheck
// Body: { checkinId: string, username: string, role: string }
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { checkinId, username, role } = body || {};
    ...
    // Remove from the set
    const removedCount = await kv.srem(`checkin:${checkinIdClean}:users`, lowerUsername);
    if (!removedCount) {
    ...
    console.error("Error executing undo checkin:", err);
    return res.status(500).json({ error: err.message || "Failed to uncheck" });
  }
}

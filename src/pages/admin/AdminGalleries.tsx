...
export default function AdminGalleries() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        "Album successfully saved is globally visible to everyone!",
        "success",
      );
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast(
        `Upload failed at photo processing. Please try again.`,
        "error",
      );
    } finally {
  ...
  return (
    <div className="relative flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 bg-white sticky top-0 z-10 w-full mb-6">
        <button
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {!editingAlbum && (
          <button
            onClick={initNew}
            disabled={loading}
            className="bg-black text-white px-4 py-2 rounded-lg font-bold hover:bg-gray-800 disabled:bg-gray-400 shadow-sm transition-all border-none cursor-pointer"
          >
            + Create Album
          </button>
        )}
      ...
      {editingAlbum ? (
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-yellow-500 focus:border-yellow-500 font-medium"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-widest mb-1">
                ...
                <select
                  ...
                  className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-sm focus:ring-yellow-500 focus:border-yellow-500 font-bold"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <label className="flex items-center gap-2 text-sm font-bold cursor-pointer">
                <input
                  type="checkbox"
                  checked={editingAlbum.showInFeed}
                  disabled={loading}
                  onChange={(e) =>
                    setEditingAlbum({
                      ...editingAlbum,
              ...
              <label className="flex items-center gap-2 text-sm font-bold cursor-pointer">
                <input
                  type="checkbox"
                  checked={editingAlbum.showInLatest}
                  disabled={loading}
                  onChange={(e) =>
                    setEditingAlbum({
                      ...editingAlbum,
                      showInLatest: e.target.checked,
                    })
                  }
                  className="rounded text-black focus:ring-black"
                />
              ...
              <label className="flex items-center gap-2 text-sm font-bold cursor-pointer">
                <input
                  type="checkbox"
                  checked={editingAlbum.allowDownload || false}
                  disabled={loading}
                  onChange={(e) =>
                    setEditingAlbum({
                      ...editingAlbum,
            ...
            <div className="pt-4 border-t border-gray-100">
              <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center mb-4 gap-2">
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-widest">
                  Images ({editingAlbum.images.length})
                </label>
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                {editingAlbum.images.map((img, idx) => (
                  <div
                    key={idx}
                    className="relative rounded-lg overflow-hidden border border-gray-200 group aspect-square bg-gray-50 w-full"
                  >
                    <img
                      src={img.url}
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs shadow-md hover:bg-red-600 z-10 border-none cursor-pointer"
                    >
                      ✕
                    </button>
                    <input
                ...
                {editingAlbum.images.length === 0 && (
                  <div className="col-span-full py-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200 text-gray-400 font-bold text-sm">
                    No photos added.
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-6 pt-4 border-t border-gray-100 sm:flex sm:justify-end font-bold">
              ...
              <button
                ...
                className="hidden md:block w-full sm:w-auto px-4 py-2 font-bold text-sm bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-lg border-none cursor-pointer"
                disabled={loading}
              >
                Cancel
              </button>
              <button
                type="submit"
                className="w-full sm:w-auto px-6 py-2 font-bold text-sm bg-black hover:bg-gray-800 text-white rounded-lg flex items-center justify-center gap-2 border-none cursor-pointer"
                disabled={loading}
              >
                {loading ? (
                  <>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {galleries.map((g: any) => {
            const firstImage =
              g.images && g.images.length > 0 ? g.images[0].url : "";
            return (
              <div
                key={g.id}
                className="bg-white border border-gray-100 rounded-xl p-3 flex flex-col justify-between shadow-sm hover:shadow-md transition-all"
              >
                <div>
                  ...
                  <h3
                    className="font-bold text-gray-950 leading-tight text-sm truncate"
                    title={g.title}
                  >
                    {g.title}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {g.showInFeed && (
                      <span className="text-[8px] font-bold bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded uppercase leading-none">
                        Feed
                      </span>
                    )}
                    {g.showInLatest && (
                      <span className="text-[8px] font-bold bg-yellow-50 text-yellow-700 px-1.5 py-0.5 rounded uppercase leading-none">
                        Latest
                      </span>
                    )}
                    {g.allowDownload && (
                      <span className="text-[8px] font-bold bg-green-50 text-green-600 px-1.5 py-0.5 rounded uppercase leading-none">
                        Download
                      </span>
                    )}
                  </div>
